...
1. [Message 与 EngineMessage](./message-and-engine-message.md): 解释 `Message`、`EngineMessage`、`runtime_messages` 的关系，以及 `Message -> EngineMessage` 的提升规则。
2. [Projection 实现细节](./projection-implementations.md): 逐个解释 `Model`、`Transcript`、`Compact`、`Debug` projection 的实现细节。
3. [模型输入与 UI 展示](./model-and-ui-mapping.md): 解释 `ModelMessage -> ChatMessage -> LLM request`，以及当前 Web UI 的展示映射。
