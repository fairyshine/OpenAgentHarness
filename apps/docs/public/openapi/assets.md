...
列出 workspace runtime 资产。返回 `kind: "runtime"` 与 `items[]`。

## `POST /assets/runtimes/upload`

上传一个 `.zip` 包作为新的 workspace runtime。Query: `name`、可选 `overwrite`。Body: `application/octet-stream`。

## `PUT /assets/runtimes/{name}`

覆盖更新一个已有 runtime 资产。Body 同上传接口。
...
列出平台 model 资产。返回 `kind: "model"` 与 `items[]`。

## `POST /assets/models/upload`

上传一个 model YAML 文件。Query: `name`、可选 `overwrite`。Body: `application/octet-stream`。
...
列出平台 tool 资产。返回 `kind: "tool"` 与 `items[]`。

## `POST /assets/tools/upload`

上传一个 tool 定义。Query: `name`、可选 `overwrite`。Body: `application/json`。
...
列出平台 skill 资产。返回 `kind: "skill"` 与 `items[]`。

## `POST /assets/skills/upload`

上传一个 skill。Query: `name`、可选 `overwrite`。Body: `application/json`。
